                        ...
                        with tabs[i]:
                            ...
                            if len(year_data) > 0:
                                ...
                                if len(high_points) >= 2:
                                    for i in range(len(high_points) - 1):
                                        ...
                                        if len(period_data) > 1:  # 确保有足够的数据点
                                            ...
                                            # 计算持续天数
                                            dd_duration = (next_high['time'] - current_high['time']).total_seconds() / (24 * 3600)
                                            ...
                                            if (abs(max_drawdown_pct) >= min_drawdown and
                                                ...
                                                dd_duration >= min_drawdown_days and
                                                dd_duration <= max_drawdown_days):
                                                ...
                                                enhanced_drawdown_events.append({
                                                    ...
                                                    'duration': dd_duration,  # 回撤持续天数（从高点到下一个新高）
                                                    'recovery_duration': recovery_duration,  # 从最低点到新高的恢复天数
                                                    'high_interval': dd_duration  # 新高之间的间隔天数
                                                })
                                
                                # 如果有特殊年份的特殊处理，确保包含最大回撤
                                special_years = [2021, 2025]
                                if year in special_years and year_max_dd < -20.0:
                                    already_added = False
                                    for event in enhanced_drawdown_events:
                                        if abs(event['max_drawdown'] - year_max_dd) < 0.1:  # 检查是否已添加相似的回撤
                                            already_added = True
                                            break
                                    ...
                                    if not already_added:
                                        ...
                                        # 手动添加最大回撤事件，尽可能包含完整信息
                                        max_dd_period_data = year_data[(year_data['candle_begin_time'] >= max_dd_start_time) & 
                                                                  (year_data['candle_begin_time'] <= max_dd_end_time)]
                                        
                                        # 找到回撤后的第一个新高点
                                        next_high_after_dd = None
                                        for hp in high_points:
                                            if hp['time'] > max_dd_end_time:
                                                next_high_after_dd = hp
                                                break
                                        
                                        if next_high_after_dd:
                                            recovery_duration = (next_high_after_dd['time'] - max_dd_end_time).total_seconds() / (24 * 3600)
                                            total_duration = (next_high_after_dd['time'] - max_dd_start_time).total_seconds() / (24 * 3600)
                                        else:
                                            recovery_duration = np.nan
                                            total_duration = max_dd_duration
                                        
                                        enhanced_drawdown_events.append({
                                            'dd_start_time': max_dd_start_time,
                                            'dd_lowest_time': max_dd_end_time,
                                            'dd_end_time': next_high_after_dd['time'] if next_high_after_dd else pd.NaT,
                                            'max_drawdown': year_max_dd,
                                            'duration': total_duration,
                                            'recovery_duration': recovery_duration,
                                            'high_interval': total_duration
                                        })
                                ...
                                if len(year_events_df) > 0:
                                    # 创建排序选项
                                    sort_options_year = ["默认排序", "按持续天数升序", "按持续天数降序", "按回撤幅度升序", "按回撤幅度降序", "按时间顺序"]
                                    ...
                                    elif selected_sort_year == "按时间顺序":
                                        year_events_df = year_events_df.sort_values(by='dd_start_time', ascending=True)
